...
def main():
    ...
    if args.loss == 'mlce' and args.task_type != 'slc':
        ...
        criterion = 0
        pass
    weight_dir = "./run/" + args.pdb_src + '-' + args.loss + str(args.batch_size) + '-' + str(args.lr) + '-' + str(args.epochs)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(set_all)):
        ...
        model = GLOBPeptide(classes=set_all.num_classes, q_encoder=args.model)
