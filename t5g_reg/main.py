...
def main():
    ...
    weight_dir = "./run/" + '-'.join([args.mode, args.gnn_type, args.loss, str(args.batch_size), str(args.lr), str(args.epochs), args.pdb_src])
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(set_all)):
        ...
        if args.model == 'mm':
            model = FusionGraph(num_layer=args.num_layer, input_dim=43, emb_dim=args.emb_dim, out_dim=set_all.num_classes, JK="last",
        ...
        for epoch in range(1, args.epochs + 1):
            if args.task_type in ('mlc', 'slc') :
                train_loss, mae, rse, pcc, kcc = train(args, epoch, model, train_loader, valid_loader, device, criterion, optimizer)
                ...
                if avg_metric > best_metric or epoch % 10 == 0:
                    if avg_metric > best_metric:
                        ...
                        logging.info(f'Per 10 epochs check, running test session')
                    
                    _, qlx_mae, qlx_rse, qlx_pcc, qlx_kcc = train(args, epoch, model, None, qlx_loader, device, None, None)
                    logging.info(f'Epoch: {epoch:03d} QLX results, mae: {qlx_mae:.3f}, rse: {qlx_rse:.3f}, pcc: {qlx_pcc:.3f}, kcc: {qlx_kcc:.3f}')
                    qlx_metric = (qlx_pcc + qlx_kcc) / (qlx_mae + qlx_rse)
                    if qlx_metric > best_qlx:
                        best_qlx = qlx_metric
                        qlx_perform_list[fold] = np.asarray([qlx_mae, qlx_rse, qlx_pcc, qlx_kcc])
                        if epoch % 10 == 0:
                            torch.save(model.state_dict(), weights_path)
                    
                    _, saap_mae, saap_rse, saap_pcc, saap_kcc = train(args, epoch, model, None, saap_loader, device, None, None)
                    logging.info(f'Epoch: {epoch:03d} SAAP results, mae: {saap_mae:.3f}, rse: {saap_rse:.3f}, pcc: {saap_pcc:.3f}, kcc: {saap_kcc:.3f}')
                    saap_metric = (saap_pcc + saap_kcc) / (saap_mae + saap_rse)
                    if saap_metric > best_saap:
                        best_saap = saap_metric
                        saap_perform_list[fold] = np.asarray([saap_mae, saap_rse, saap_pcc, saap_kcc])
