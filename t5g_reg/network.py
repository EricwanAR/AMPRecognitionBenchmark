...
from model_ex.egnn import EGNN
...
class GNN(nn.Module):
    def __init__(self, num_layer, input_dim, emb_dim, JK="last", drop_ratio=0, gnn_type="gin"):
        ...
        self.embedder = Linear(input_dim, emb_dim)

        for layer in range(num_layer):
            if gnn_type == "gin":
                # self.gnns.append(GINConv(nn.Sequential(nn.Linear(in_dim, emb_dim), nn.BatchNorm1d(emb_dim), nn.ReLU(),
                #                                        nn.Linear(emb_dim, emb_dim))))
                self.gnns.append(GINConv(nn.Sequential(nn.Linear(emb_dim, emb_dim), GraphNorm(emb_dim), nn.ReLU(),
                ...
                conv = GPSConv(emb_dim, GINConv(nn_), heads=4)
                self.gnns.append(conv)
            elif gnn_type == "gcn":
                self.gnns.append(GraphConv(emb_dim, emb_dim))
            elif gnn_type == "gat":
...
class FusionGraph(nn.Module):
    def __init__(self, num_layer, input_dim, emb_dim, out_dim, JK="last", drop_ratio=0.5, graph_pooling="attention",
        ...
        if gnn_type == 'e3':
            self.gnn = EGNN(in_node_nf=input_dim-3, hidden_nf=emb_dim-3, out_node_nf=emb_dim-3, n_layers=num_layer)
        else:
