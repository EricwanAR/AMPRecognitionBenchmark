...
parser.add_argument('--in-channels', dest='in_channels', type=int, default=3)
...
parser.add_argument('--exclude-channel', dest='exclude_channel', type=int, default=None)
...
def main():
    ...
    if args.loss == 'mlce' and args.task_type != 'slc':
        ...
        criterion = 0
        pass
    if args.exclude_channel is None:
        args.in_channels = 4
    weight_dir = "./run/" + str(args.exclude_channel) + '-' + args.loss + str(args.batch_size) + '-' + str(args.lr) + '-' + str(args.epochs) + args.pdb_src
    ...
    if args.task_type == 'mlc':
        set_all = MDataset(threshold=args.threshold, mode='train', max_length=args.max_length, pdb_src=args.pdb_src, 
                           data_ver=args.data_ver, exclude_channel=args.exclude_channel)
    # else:
